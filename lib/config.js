...
export const DEFAULT_CONFIG = {
  ...
  // Scope every metric to Online Store orders only (Order.sourceName === "web").
  // Non-web orders (POS, draft, other sales channels) are dropped at aggregation
  // time and reported as a diagnostic count.
  onlineStoreOnly: true,
  // When true, the aggregator also breaks the metric down per order landing-page
  // path. Forces the (expensive) customerJourneySummary into the bulk export.
  // Used by the UPT Diagnosis "by landing page" view.
  landingBreakdown: false,
};
...
export function normalizeConfig(input = {}) {
  ...
  return {
    ...
    onlineStoreOnly: c.onlineStoreOnly === undefined ? DEFAULT_CONFIG.onlineStoreOnly : Boolean(c.onlineStoreOnly),
    landingBreakdown: Boolean(c.landingBreakdown),
  };
}
