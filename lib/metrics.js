...
export function createAccumulator(cfg) {
  ...
  return {
    ...
    finalize() {
      ...
      // Discount-code mix (window-level). `ordersWithCode` counts orders that
      // used at least one discount code; `discountedOrders` also includes orders
      // discounted with no code (automatic discounts, totalDiscounts > 0).
      ...
      for (const o of orders.values()) {
        ...
        if (hasCode) {
          ordersWithCode += 1;
          // De-dupe codes within a single order so one order counts once per code.
          for (const code of new Set(o.discountCodes)) {
            let c = codeMap.get(code);
            if (!c) {
              c = { code, orders: 0, sales: 0, discountAmount: 0 };
              codeMap.set(code, c);
            }
            c.orders += 1;
            c.sales += o.sales;
        ...
        if (isDiscounted) {
          discountedOrders += 1;
          discountAmountTotal += o.totalDiscounts;
        }
      ...
      return {
        ...
        discounts: {
          ...
          ordersWithCode,
          discountedOrders,
          discountAmount: round(discountAmountTotal, 2),
          codes,
        },
        ...(landingPages
