...
export function createAccumulator(cfg) {
  ...
  const orders = new Map(); // orderId -> { createdAt, landingPage, units, sales }
  let currency = null;

  function ensure(id) {
    let o = orders.get(id);
    if (!o) {
      o = { createdAt: null, landingPage: null, units: 0, sales: 0 };
      orders.set(id, o);
    }
    return o;
  }
  ...
  return {
    ...
    finalize() {
      ...
      const totals = { units: 0, transactions: 0, sales: 0 };
      let ordersMatched = 0;
      ...
      for (const o of orders.values()) {
        ...
        if (!b) {
          b = { key, label, units: 0, transactions: 0, sales: 0 };
          bucketMap.set(key, b);
        }
        b.units += o.units;
        b.transactions += 1;
